...
class Render(BaseCollection):
    def __init__(
        ...
        if coll and coll.Brender.flag:
            ...
            self.set_collection(viewport, animation=animation, run_render=run_render)
    ...
    def set_collection(self, viewport, animation=False, run_render=True):
        """
    ...
    def set_compute_device_type(self, compute_device_type):
        bpy.context.preferences.addons[
            "cycles"
        ].preferences.compute_device_type = compute_device_type.upper()
    ...
    def studiolight(self, studiolight):
        bpy.context.scene.display.shading.studio_light = "%s" % studiolight
    ...
    def update_camera(self, width=None, height=None, depth=0):
        if width is not None:
            ...
            self.camera.lens = (
                self.camera.obj.data.sensor_width * (self.distance - depth) / width
            )
            self.resolution = [self.resolution[0], self.resolution[0] * height / width]
        self.camera.location = self.center + self.viewport * self.distance
    ...
    def run(self, batoms, center=None, padding=None, canvas=None, **kwargs):
        """
        render the model and export result
        """
        self.batoms = batoms
        self.set(**kwargs)
        self.set_viewport_distance_center(center=center, canvas=canvas, padding=padding)
        bpy.context.scene.camera = self.camera.obj
        #
        ...
        if self.run_render:
            ...
            bpy.ops.wm.save_as_mainfile(
                "EXEC_SCREEN", filepath="{0}.blend".format(self.output)
            )
