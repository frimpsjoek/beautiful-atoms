...
class Render(bpy.types.PropertyGroup):
    ...
    distance: FloatProperty(
        name="distance", description="Distance from camera", default=-1
    )
    padding: FloatVectorProperty(name="padding", default=[1, 1, 1, 1], size=4)

    light_ui_list_index: IntProperty(name="light_ui_list_index", default=0)
    camera_ui_list_index: IntProperty(name="camera_ui_list_index", default=0)
    # colleciton properties
    light_settings: CollectionProperty(name="lightsetting", type=LightSetting)
    camera_settings: CollectionProperty(name="camerasetting", type=CameraSetting)
