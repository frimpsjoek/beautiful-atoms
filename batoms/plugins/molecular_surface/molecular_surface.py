...
class MolecularSurface(BaseObject):
    def __init__(
        ...
        self.settings = MolecularSurfaceSettings(self.label, parent=self)
    ...
    def calc_isosurface(
        self, volume, level, spacing, origin=[0, 0, 0], color=(0.2, 0.1, 0.9, 1.0)
    ):
        """
    ...
    def draw_SAS(self, ms, parallel=1):
        ...
        probe = ms.probe
        logger.debug("Resolution: {:1.3f}, Probe: {:1.3}".format(resolution, probe))
        tstart = time()
        ...
        indices_sas, volume_sas = self.calc_power_distance(
            self.meshgrids, positions, radii, parallel=parallel
        )
        volume = volume_sas.reshape(self.shape)
        isosurface = self.calc_isosurface(
            volume, 5, spacing=self.get_space(resolution), origin=self.box_origin
        )
        ...
        if ms.color_by != "None":
            ...
            if ms.color_by.upper() == "ELECTROSTATIC_POTENTIAL":
                ...
                # scaled positions
                scaled_verts = Cell(self.batoms.cell).scaled_positions(
                    isosurface["vertices"]
                )
                data = map_volumetric_data(
                    self.batoms.volumetric_data[ms.color_by], scaled_verts
                )
            # normalize
            ...
            obj.data.attributes.new(
                name="{}_data".format(ms.color_by), type="FLOAT", domain="POINT"
            )
            set_mesh_attribute(obj, "{}_data".format(ms.color_by), data)
            color_by_attribute = {
                "attribute_name": "{}_data".format(ms.color_by),
                "ValToRGB": [ms.color1[:], ms.color2[:]],
            }
        else:
            color_by_attribute = None
        mat = self.build_materials(
            sas_name,
    ...
    def draw_SES(self, ms, parallel=1):
        ...
        indices_sas1, tmp = self.calc_power_distance(
            self.meshgrids[indices], positions, radii, parallel=parallel
        )
        ...
        indices_ses, volume_ses = self.calc_power_distance(
            self.meshgrids[indices], vertices, radii, parallel=parallel
        )
    ...
    def calc_power_distance(self, points, positions, radii, k=1, parallel=1):
        """
        Algorithm:
    ...
    def draw_SES_mb(
        ...
        radii = np.array(self.batoms.radii_vdw) + self.probe
        obj = self.build_SAS(
            resolution=resolution, threshold=threshold, stiffness=stiffness
        )
        ...
        indices, mask1, mask2, mask3 = self.SES_map_to_atom(
            vertices, positions, radii, eps
        )
        # return
        # refine_normal = False
        if refine_normal:
            normals = self.SES_refine_normal(
    ...
    def SES_map_to_atom(self, vertices, positions, radii, eps, obj=None):
        ...
        indices31, origins_probe3 = check_origin_3(
            vertices3,
            origins0,
            origins1,
            origins2,
        ...
        indices21, origins_probe2 = check_origin_2(
            vertices2,
            origins0,
            origins1,
    ...
    def SES_subdivide(self, positions, radii, eps, number_cuts=2):
        ...
        mask = np.where(
            (((delta0 < eps)) | (delta1 < eps)) & (lengths > eps / 2 / number_cuts),
            True,
            False,
        )
        ...
        bpy.ops.mesh.subdivide(
            number_cuts=int(number_cuts), smoothness=0, fractal_along_normal=0
        )
        # return
        bpy.ops.object.mode_set(mode="OBJECT")

    def SES_refine_normal(
        self, vertices, normals, positions, indices, indices1, indices2, indices3
    ):
        # -----------------------------------------
