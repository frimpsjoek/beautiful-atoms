...
class LatticePlane(PluginObject):
    def __init__(
        ...
        self.settings = LatticePlaneSettings(self.label, parent=self)
    ...
    def build_plane(self, bcell, include_center=False):
        ...
        for p in self.settings:
            ...
            vertices, edges, faces = faces_from_vertices(
                intersect_points, normal, include_center=include_center, scale=p.scale
            )
    ...
    def build_slicing(self, name, plane, bcell, cuts=None, cmap="Spectral"):
        ...
        me.attributes.new(name=plane["color_by"], type="FLOAT", domain="POINT")
    ...
    def draw(
        self, plane_name="ALL", no=None, cuts=None, cmap="bwr", include_center=False
    ):
        """Draw plane
        no: int
        ...
        for species, plane in planes.items():
            ...
            if plane["boundary"]:
                ...
                if plane["color_by"] != "None":
                    name = "%s_%s_%s" % (self.label, self.name, species)
                    self.build_slicing(
                        name, plane, self.batoms.cell, cuts=cuts, cmap=cmap
                    )
                    color_by_attribute = {
                        "attribute_name": plane["color_by"],
                        "ValToRGB": [plane["color1"], plane["color2"]],
                    }
                else:
                    color_by_attribute = None
...
def convexhull(planes, point):
    ...
    for name, plane in planes.items():
        ...
        x2 = np.dot(np.array([0, 0, 0]), plane["normal"]) - np.dot(
