...
class BATOMS_MT_lattice_plane_context_menu(Menu):
    ...
    def draw(self, _context):
        layout = self.layout
        op = layout.operator(
            "plane.lattice_plane_add", icon="ADD", text="Add Lattice Plane"
        )
        layout.separator()
        op = layout.operator(
            "plane.lattice_plane_remove", icon="X", text="Delete All Lattice Plane"
        )
        op.all = True
...
class BATOMS_PT_lattice_plane(Panel):
    ...
    def draw(self, context):
        ...
        if kb:
            ...
            op = layout.operator(
                "plane.lattice_plane_draw", icon="GREASEPENCIL", text="Draw"
            )
