...
class Cavity(ObjectGN, PluginObject):
    def __init__(
        ...
        if not flag:
            ...
            self.settings = CavitySettings(self.label, batoms=batoms, parent=self)
        else:
            self.settings = CavitySettings(self.label, batoms=batoms, parent=self)
    ...
    def base_meshgrids(self, grids):
        x, y, z = np.meshgrid(
            grids[0], grids[1], grids[2], indexing="ij"
        )  # , sparse=True)
    ...
    def build_cavity(self):
        ...
        for i in range(ns):
            ...
            if not has_r and ncav == 0:
                ...
                cav = {
                    "min": np.floor(spheres["radii"][i]),
                    "max": np.ceil(spheres["radii"][i]),
                    "color": basic_colors[color_names[ic]],
                }
        ...
        for i in range(ncollection):
            ...
            indices = np.where(
                (cavities["radii"] > cav.min) & (cavities["radii"] < cav.max)
            )[0]
        ...
        cavities.update(
            {
                "centers": np.array([cavities["centers"]]),
    ...
    def refine_spheres(self, center):
        ...
        for i in range(3):
            grid = (
                np.arange(-npoint[i], npoint[i] + 1) / npoint[i] * self.resolution / 2
            )
            grids.append(grid)
    ...
    def build_object(self, cavity_datas, attributes={}):
        ...
        if len(cavity_datas["centers"].shape) == 2:
            self._trajectory = {
                "centers": np.array([cavity_datas["centers"]]),
            }
    ...
    def add_geometry_node(self, spname, instancer):
        ...
        links.new(GroupInput.outputs[1], CompareSpecies.inputs[0])
        links.new(GroupInput.outputs[2], BoolShow.inputs[0])
        links.new(GroupInput.outputs[3], InstanceOnPoint.inputs["Scale"])
