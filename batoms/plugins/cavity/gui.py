...
class BATOMS_MT_cavity_context_menu(Menu):
    ...
    def draw(self, _context):
        layout = self.layout
        op = layout.operator("surface.cavity_add", icon="ADD", text="Add cavity")
        layout.separator()
        op = layout.operator(
            "surface.cavity_remove", icon="X", text="Delete All cavity"
        )
        op.all = True
...
class BATOMS_UL_cavity(UIList):
    def draw_item(
        ...
        if self.layout_type in {"DEFAULT", "COMPACT"}:
            ...
            split.prop(cavity, "name", text="", emboss=False, icon=custom_icon)
...
class BATOMS_PT_cavity(Panel):
    ...
    def draw(self, context):
        ...
        row.template_list(
            "BATOMS_UL_cavity", "", ba, "settings", ba, "ui_list_index", rows=rows
        )
        ...
        col.menu("BATOMS_MT_cavity_context_menu", icon="DOWNARROW_HLT", text="")
        ...
        if kb:
            ...
            col.separator()
        op = layout.operator("surface.cavity_draw", icon="GREASEPENCIL", text="Draw")
