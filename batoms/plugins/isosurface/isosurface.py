...
class Isosurface(BaseObject):
    def __init__(
        ...
        self.settings = IsosurfaceSettings(self.label, batoms=batoms, parent=self)
    ...
    def build_isosurface(self, cell):
        ...
        for iso in self.settings.bpy_setting:
            if iso.volumetric_data == "":
                volume = self.batoms.volumetric_data[
                    self.batoms.volumetric_data.bpy_setting[0].name
                ]
            else:
            ...
            if iso.color_by != "None":
                ...
                color_by_attribute = {
                    "attribute_name": "{}_data".format(iso.color_by),
                    "ValToRGB": [iso.color1[:], iso.color2[:]],
                }
            else:
    ...
    def draw(self, isosurface_name="ALL"):
        ...
        for name, isosurface_data in isosurface.items():
            ...
            if isosurface_data["attribute_data"] is not None:
                ...
                set_mesh_attribute(
                    obj,
                    isosurface_data["color_by_attribute"]["attribute_name"],
                    isosurface_data["attribute_data"],
                )
            # material
            mat = self.build_materials(
                name,
                isosurface_data["color"],
...
def calc_isosurface(
    volumetric_data, cell, level, gradient_direction="descent", step_size=1
):
    """
    ...
    scaled_verts, faces, normals, values = measure.marching_cubes(
        volumetric_data,
        level=level,
        spacing=spacing,
