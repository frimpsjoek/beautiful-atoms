...
class BATOMS_MT_isosurface_context_menu(Menu):
    ...
    def draw(self, _context):
        layout = self.layout
        op = layout.operator(
            "surface.isosurface_add", icon="ADD", text="Add Isosurface"
        )
        layout.separator()
        op = layout.operator(
            "surface.isosurface_remove", icon="X", text="Delete All Isosurface"
        )
        op.all = True
...
class BATOMS_PT_isosurface(Panel):
    ...
    def draw(self, context):
        ...
        row.template_list(
            "BATOMS_UL_isosurface", "", ba, "settings", ba, "ui_list_index", rows=rows
        )
