...
class BATOMS_MT_highlight_context_menu(Menu):
    ...
    def draw(self, _context):
        layout = self.layout
        op = layout.operator("batoms.highlight_add", icon="ADD", text="Add highlight")
        layout.separator()
        op = layout.operator(
            "batoms.highlight_remove", icon="X", text="Delete All highlight"
        )
        op.all = True
...
class BATOMS_UL_highlight(UIList):
    def draw_item(
        ...
        if self.layout_type in {"DEFAULT", "COMPACT"}:
            ...
            split.prop(highlight, "name", text="", emboss=False, icon=custom_icon)
...
class BATOMS_PT_highlight(Panel):
    ...
    def draw(self, context):
        ...
        row.template_list(
            "BATOMS_UL_highlight", "", ba, "settings", ba, "ui_list_index", rows=rows
        )
        ...
        col.menu("BATOMS_MT_highlight_context_menu", icon="DOWNARROW_HLT", text="")
        ...
        if kb:
            ...
            col.separator()
        op = layout.operator("batoms.highlight_draw", icon="GREASEPENCIL", text="Draw")
