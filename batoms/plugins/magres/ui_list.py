...
class BATOMS_MT_magres_context_menu(Menu):
    ...
    def draw(self, _context):
        layout = self.layout
        op = layout.operator(
            "surface.magres_add", icon="ADD", text="Add Molecular Surface"
        )
        layout.separator()
        op = layout.operator(
            "surface.magres_remove", icon="X", text="Delete All Molecular Surface"
        )
        op.all = True
...
class BATOMS_PT_magres(Panel):
    ...
    def draw(self, context):
        ...
        row.template_list(
            "BATOMS_UL_magres", "", ba, "settings", ba, "ui_list_index", rows=rows
        )
