...
class BATOMS_MT_crystal_shape_context_menu(Menu):
    ...
    def draw(self, _context):
        layout = self.layout
        op = layout.operator(
            "plane.crystal_shape_add", icon="ADD", text="Add Crystal Shape"
        )
        layout.separator()
        op = layout.operator(
            "plane.crystal_shape_remove", icon="X", text="Delete All Crystal Shape"
        )
        op.all = True
...
class BATOMS_PT_crystal_shape(Panel):
    ...
    def draw(self, context):
        ...
        if kb:
            ...
            op = layout.operator(
                "plane.crystal_shape_draw", icon="GREASEPENCIL", text="Draw"
            )
