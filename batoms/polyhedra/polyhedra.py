...
class Polyhedra(ObjectGN):
    ...
    def __init__(
        ...
        self.settings = PolyhedraSettings(self.label, batoms=batoms, polyhedras=self)
        flag = self.load()
    ...
    def build_object(self, datas, attributes={}):
        ...
        if len(datas["vertices"].shape) == 2:
            self._trajectory = {
                "vertices": np.array([datas["vertices"]]),
        ...
        mesh.from_pydata(vertices, datas["edges"], datas["faces"])
        mesh.update()
    ...
    def add_geometry_node(self, sp):
        ...
        links.new(GroupInput.outputs[3], CompareSpecies.inputs[0])
        # face
        CompareSpeciesFace = get_node_by_name(
            nodes,
        ...
        links.new(GroupInput.outputs[4], CompareSpeciesFace.inputs[0])
        #
        setMaterialIndex = get_node_by_name(
            nodes,
            "%s_setMaterialIndex_%s" % (self.label, sp["species"]),
            "GeometryNodeSetMaterialIndex",
        )
        setMaterialIndex.inputs[2].default_value = self.settings.materials[
            sp["species"]
        ][1]
        links.new(previousNode, setMaterialIndex.inputs["Geometry"])
        links.new(CompareSpeciesFace.outputs[0], setMaterialIndex.inputs["Selection"])
        links.new(setMaterialIndex.outputs["Geometry"], GroupOutput.inputs["Geometry"])
    ...
    def set_realize_instances(self, realize_instances):
        ...
        RealizeInstances = get_node_by_name(
            nodes, "%s_RealizeInstances" % self.label, "GeometryNodeRealizeInstances"
        )
        if not realize_instances:
            ...
            self.gn_node_group.links.new(
                setMaterialIndex.outputs[0], nodes[1].inputs[0]
            )
        else:
            self.gn_node_group.links.new(
                setMaterialIndex.outputs[0], RealizeInstances.inputs[0]
            )
            self.gn_node_group.links.new(
    ...
    def update(
        ...
        for f in range(nframe):
            ...
            polyhedra_kinds = self.calc_polyhedra_data(
                bondlists, species, positions, arrays["model_style"][show]
            )
            if f == 0:
    ...
    def calc_polyhedra_data(self, bondlists, species, positions, model_styles):
        """ """
