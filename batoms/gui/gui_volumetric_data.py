...
class BATOMS_MT_volumetric_data_context_menu(Menu):
    ...
    def draw(self, _context):
        layout = self.layout
        op = layout.operator("batoms.volumetric_data_add", icon="ADD", text="Add Data")
        layout.separator()
        op = layout.operator(
            "batoms.volumetric_data_remove", icon="X", text="Delete All Data"
        )
        op.all = True
...
class BATOMS_UL_volumetric_data(UIList):
    def draw_item(
        ...
        if self.layout_type in {"DEFAULT", "COMPACT"}:
            ...
            split.prop(volumetric_data, "name", text="", emboss=False, icon=custom_icon)
...
class BATOMS_PT_volumetric_data(Panel):
    ...
    def draw(self, context):
        ...
        row.template_list(
            "BATOMS_UL_volumetric_data",
            "",
            ba,
            "settings_volumetric_data",
            ba,
            "ui_list_index_volumetric_data",
            rows=rows,
        )
        ...
        col.menu(
            "BATOMS_MT_volumetric_data_context_menu", icon="DOWNARROW_HLT", text=""
        )
